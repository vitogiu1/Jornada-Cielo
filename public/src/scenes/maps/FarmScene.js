...
export class FarmScene extends Phaser.Scene {
  ...
  create() {
    ...
    mapMusics.forEach((key) => {
      ...
      if (key !== "farm_music" && this.sound.get(key)?.isPlaying) {
        this.sound.get(key).stop();
      }
    });

    // Inicia a música da fazenda se não estiver ativa.
    if (!this.sound.get("farm_music")?.isPlaying) {
      this._playMapMusic();
    } else {
      this.backgroundMusic = this.sound.get("farm_music");
    }

    // Registra evento de shutdown para parar a música ao fechar a cena (não afeta sleep).
    this.events.once("shutdown", () => {
      this.sound.getAll("farm_music").forEach((s) => s.stop());
      if (this.backgroundMusic) this.backgroundMusic.stop();
    });

    // Se a cena for pausada a música para.
  ...
  /** Inicia (ou reinicia) a música de fundo de forma segura. */
  _playMapMusic() {
    const vol = this.registry.get("musicVolume") ?? 0.3;
    let existing = this.sound.get("farm_music");

    // Recicla se já existe a música em cache
    if (existing) {
      this.backgroundMusic = existing;
    } else {
      this.backgroundMusic = this.sound.add("farm_music", {
        loop: true,
      });
    }
