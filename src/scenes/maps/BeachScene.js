...
export class BeachScene extends Phaser.Scene {
  ...
  create() {
    ...
    mapMusics.forEach((key) => {
      ...
      if (key !== "beach_music" && this.sound.get(key)?.isPlaying) {
    ...
    addEntryTrigger(mapX + 250, mapY + 300, 64, 64, "QuiosquePraiaScene");
    addEntryTrigger(mapX + 700, mapY + 350, 64, 64, "LojaPraiaScene");
    addEntryTrigger(mapX + 200, mapY + 650, 64, 64, "QuiosDoisPraiaScene");
    addEntryTrigger(mapX + 600, mapY + 700, 64, 64, "QuiosSurfScene");
    addEntryTrigger(mapX + 850, mapY + 600, 64, 64, "QuiosTresScene");

    // Ao retornar de interior, reseta transição e afasta o player da porta para evitar loop.
    this.events.on("wake", () => {
      this.isTransitioning = false;
      this.player.y += 40;
      this.cameras.main.fadeIn(500);
